...
async function addBlock(page, buttonName) {
  const colMenu = page
...
test.describe("UAT Publish Probe - Two Blocks", () => {
  test("Publish page with two blocks", async ({ page }) => {
    ...
    await test.step("Create Standard Page", async () => {
      ...
      await page.waitForLoadState("domcontentloaded");
      await page.getByRole("textbox", { name: "Page Title" }).fill("UAT Two Blocks Test");
    });

    await test.step("Add 1-Column Section", async () => {
      await addUatSection(page, "UAT Two Blocks Section");
    });

    await test.step("Add Image Block", async () => {
      await addBlock(page, "Add Image Block");
      ...
      await modal.locator(".media-library-item, .js-media-library-item").first().click({ force: true });
      await modal.getByRole("button", { name: "Insert selected" }).click();
      await modal.waitFor({ state: "hidden", timeout: 20000 });
      await collapseUat(page);
    });

    await test.step("Add Accordion Block", async () => {
      await addBlock(page, "Add Accordion Block");
      await page.getByRole("button", { name: "Add Accordion Item" }).last().click();
      await page.waitForTimeout(2000);
      await page.locator('textarea[name*="field_mtpc_accordion_title"]').last().fill("Two Block Accordion Item");
      await page.locator('input[name*="field_mtpc_accordion_expended"]').last().setChecked(true);
      await page.getByRole("textbox", { name: "Rich Text Editor. Editing" }).last().fill("<p>Two block accordion content</p>");
      await collapseUat(page);
    });
    ...
    await test.step("Verify Frontend", async () => {
      ...
      await expect(page.getByText("Two Block Accordion Item", { exact: true })).toBeVisible({
        timeout: 30000,
      });
    });
  });
