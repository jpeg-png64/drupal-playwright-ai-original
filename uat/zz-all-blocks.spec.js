...
test.describe("All Blocks Combined (UAT)", () => {
  test("All blocks on one page @combined", async ({ page }) => {
    ...
    await test.step("Add Next & Previous Block", async () => {
      ...
      const nextLink = page.locator('input[data-drupal-selector*="field-mtpc-next-link"][data-drupal-selector$="uri"]');
      await nextLink.click();
      await nextLink.fill("Content Template");
      await page.waitForTimeout(2500);
      await page.locator(".ui-autocomplete:visible li.ui-menu-item").first().click();

      const prevLink = page.locator('input[data-drupal-selector*="field-mtpc-previous-link"][data-drupal-selector$="uri"]');
      await prevLink.click();
      await prevLink.fill("Content Template");
      await page.waitForTimeout(2500);
      await page.locator(".ui-autocomplete:visible li.ui-menu-item").first().click();
    });
    await test.step("Collapse Block", async () => { await fastCollapseCurrentBlock(page); });
    ...
    await test.step("Publish Page", async () => {
      await page.waitForLoadState("networkidle");
      await expect(page.getByRole("textbox", { name: "Section Name" }).first()).toBeVisible({ timeout: 15000 });
