...
async function createPublishedPage(page, title) {
...
test.describe("Next & Previous Block", () => {
  test("Default", async ({ page }) => {
    ...
    await test.step("Create link target pages", async () => {
      await createPublishedPage(page, "Next Target");
      await createPublishedPage(page, "Previous Target");
    });
    ...
    await test.step("Add Next & Previous Block", async () => {
      await addNextPreviousBlock(page, {
        nextSearch: "Next Target",
        prevSearch: "Previous Target",
      });
    });
  ...
  test("All configurable fields", async ({ page }) => {
    ...
    await test.step("Create link target pages", async () => {
      await createPublishedPage(page, "Next Target Colors");
      await createPublishedPage(page, "Previous Target Colors");
    });
    ...
    await test.step("Add Block 1 - custom labels + colors", async () => {
      await addNextPreviousBlock(page, {
        nextSearch: "Next Target Colors",
        prevSearch: "Previous Target Colors",
        nextTitle: "Next Page",
